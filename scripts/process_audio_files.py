...
def main():
    ...
    try:
        ...
        print(f"\n3. Selecting and flattening features:")
        selected = []
        for group in feature_groups:
            if group in features_df.columns.levels[0]:
                group_df = features_df[group].copy()

                # Flatten multi-level columns into simple names
                # MultiIndex structure: (statistic, number) after selecting group
                # Example: ('mean', '01') -> 'mfcc_mean_01' (prepend group name)
                new_cols = []
                for col in group_df.columns:
                    if isinstance(col, tuple):
                        # Join all parts with underscore, removing empty parts
                        parts = [str(p).strip() for p in col if str(p).strip()]
                        # Prepend the feature group name
                        new_cols.append(f"{group}_{'_'.join(parts)}")
                    else:
                        new_cols.append(f"{group}_{col}")

                group_df.columns = new_cols
                selected.append(group_df)
                print(f"   {group}: {len(new_cols)} features")
